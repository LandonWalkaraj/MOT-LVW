...
DEFAULT_OUTPUT_ROOT = v5.PROJECT_ROOT / "outputs" / "benchmarks" / "lorat-v7"
EXECUTION_MODE = v7.V7_EXECUTION_MODE

STOP_REQUESTED = False


@dataclass(frozen=True)
class V7TimingResult:
    sequence: str
    lorat_config: str
    backbone: str
    input_size: int
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "V7-only LoRAT benchmark for Week 1/2 metrics: bounding-box timing, "
            "small-object area reliability, FPS/memory scaling, and shared-frame ViT proof."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Run one or more LoRAT configs under V7. Use all to include B/L/g and 224/378.",
    )
    ...
    parser.add_argument("--v7-frame-size", type=int, default=0)
    parser.add_argument("--v7-head-rank", type=int, default=v5.DEFAULT_LORAT_MEMORY_SLOTS)
    parser.add_argument("--v7-head-hidden-dim", type=int, default=256)
    parser.add_argument("--v7-head-lora-rank", type=int, default=16)
    parser.add_argument("--v7-head-weights", type=Path)
    parser.add_argument("--v7-search-radius-factor", type=float, default=2.25)
    parser.add_argument("--v7-min-confidence", type=float, default=0.48)
    parser.add_argument("--v7-template-update-rate", type=float, default=0.08)
    parser.add_argument("--v7-template-update-min-confidence", type=float, default=0.58)
    parser.add_argument("--v7-score-reduction", choices=("max", "mean"), default="max")
    ...
    parser.add_argument("--fixed-lorat-box-size", dest="lorat_fixed_box_size", action="store_true", default=v5.DEFAULT_LORAT_FIXED_BOX_SIZE)
    ...
    parser.add_argument("--v7-primary-heads-per-track", type=int, default=v7.DEFAULT_V7_PRIMARY_HEADS_PER_TRACK)
    parser.add_argument("--v7-recovery-heads-per-track", type=int, default=v7.DEFAULT_V7_RECOVERY_HEADS_PER_TRACK)
    parser.add_argument("--v7-recovery-interval", type=int, default=v7.DEFAULT_V7_RECOVERY_INTERVAL)
    parser.add_argument("--v7-recovery-min-confidence", type=float, default=v7.DEFAULT_V7_RECOVERY_MIN_CONFIDENCE)
    parser.add_argument("--v7-recovery-min-assignment-score", type=float, default=v7.DEFAULT_V7_RECOVERY_MIN_ASSIGNMENT_SCORE)
    parser.add_argument("--v7-recovery-min-assignment-margin", type=float, default=v7.DEFAULT_V7_RECOVERY_MIN_ASSIGNMENT_MARGIN)
    parser.add_argument("--v7-recovery-stale-head-frames", type=int, default=v7.DEFAULT_V7_RECOVERY_STALE_HEAD_FRAMES)
...
def run_label(args: argparse.Namespace, sequences: Sequence[Path], configs: Sequence[str], track_counts: Sequence[int]) -> str:
    ...
    return bench.slugify(f"v7_{base}")


def default_output_paths(args: argparse.Namespace, label: str) -> V7OutputPaths:
    ...
    for path in (timing_csv, area_csv, observations_csv, full_observations_csv, identity_csv, week2_proof_csv, debug_csv, summary_md):
        path.parent.mkdir(parents=True, exist_ok=True)
    return V7OutputPaths(
        run_root=run_root,
...
def preview_video_path(paths: V7OutputPaths, sequence: str, config: str, target_tracks: int, max_frames: int) -> Path:
    frame_part = f"frames{max_frames}" if max_frames > 0 else "full"
    name = bench.slugify(f"{sequence}_{config}_v7_N{target_tracks}_{frame_part}_preview") + ".mp4"
...
def collect_sampled_observations(
    ...
    previous_identity_bboxes: Dict[int, v5.BBox],
    ...
    for track in tracks:
        ...
        if identity_sampled:
            ...
            if previous_bbox is None:
                ...
                center_jump_px = v5.center_distance(previous_bbox, track.bbox)
...
def summarize_identity(identity_rows: Sequence[IdentityObservation]) -> List[Dict[str, object]]:
    grouped: Dict[Tuple[str, str, str, int], List[IdentityObservation]] = {}
    for row in identity_rows:
        key = (row.sequence, row.lorat_config, row.execution_mode, row.target_tracks)
        grouped.setdefault(key, []).append(row)
    summaries: List[Dict[str, object]] = []
    for (sequence, lorat_config, execution_mode, target_tracks), rows in sorted(grouped.items()):
        summaries.append(
            {
                "sequence": sequence,
                "lorat_config": lorat_config,
                ...
                "target_tracks": target_tracks,
                "samples": len(rows),
                "correct_rate": sum(1 for row in rows if row.correct_object) / len(rows) if rows else None,
                "jump_rate": sum(1 for row in rows if row.identity_jump) / len(rows) if rows else None,
                "occluded_rate": sum(1 for row in rows if row.occluded) / len(rows) if rows else None,
                "mean_iou": statistics.fmean(row.own_iou for row in rows) if rows else None,
            }
        )
    return summaries
...
def parse_week2_proof_lines(lines: Sequence[str]) -> List[Dict[str, str]]:
    if not lines:
        return []
    text = v7.WEEK2_PROOF_LOG_HEADER + "".join(lines)
...
def profile_ms_per_update(runtime_status: v5.RuntimeStatus, bucket: str) -> Optional[float]:
    value = getattr(runtime_status, f"v7_profile_{bucket}_ms_per_update", None)
...
def run_benchmark_case(
    ...
    debug_csv: Path,
) -> Optional[Tuple[V7TimingResult, List[bench.AreaObservation], List[bench.AreaObservation], List[IdentityObservation]]]:
    ...
    backend = v7.create_backend(run_args, source, target_tracks)
    preview_writer = None
    sampled_area: List[bench.AreaObservation] = []
    full_area: List[bench.AreaObservation] = []
    identity_rows: List[IdentityObservation] = []
    ...
    print(
        f"Starting V7 {sequence_path.name} {lorat_config} N={target_tracks}: "
    ...
    try:
        ...
        if args.save_video and preview_path is not None:
            ...
            preview_writer.write(v5.draw_tracks(init_frame_image, backend.tracks, init_frame, backend.backend_name))

        print(
            f"[V7 {sequence_path.name} {lorat_config} N={target_tracks}] "
        ...
        for image_index in range(init_index + 1, end_index + 1):
            ...
            if preview_writer is not None:
                ...
                preview_writer.write(v5.draw_tracks(frame, backend.tracks, frame_number, backend.backend_name, status_lines))
        ...
        append_week2_proof_rows(proof_rows, sequence_path.name, lorat_config, target_tracks, len(backend.tracks), backend.week2_proof_lines)
        backend.close()
        if preview_writer is not None:
...
def write_timing_csv(path: Path, rows: Sequence[V7TimingResult]) -> None:
    fieldnames = [
        "sequence",
        "lorat_config",
        "backbone",
...
def write_week2_proof_csv(path: Path, rows: Sequence[Dict[str, object]]) -> None:
    proof_fields = next(csv.reader([v7.WEEK2_PROOF_LOG_HEADER.strip()]))
    fieldnames = ["sequence", "lorat_config", "target_tracks", "actual_tracks"] + proof_fields
    with path.open("w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        for row in rows:
...
def capacity_rows(timing_rows: Sequence[V7TimingResult]) -> List[Dict[str, object]]:
    grouped: Dict[Tuple[str, str, str], List[V7TimingResult]] = {}
    for row in timing_rows:
        grouped.setdefault((row.sequence, row.lorat_config, row.gpu_profile), []).append(row)
    result: List[Dict[str, object]] = []
    for (sequence, lorat_config, gpu_profile), rows in sorted(grouped.items()):
...
def write_summary_md(
    ...
    lines = [
        ...
        "- V7 FPS profile values are elapsed milliseconds per tracked update frame, aggregated from tracker-side timers.",
        "",
        "## Output Files",
        "",
    ...
    lines.append(
        "| Sequence | Config | N Target | N Actual | FPS Track | Track ms/box | Mean IoU | IoU@0.50 | Peak GPU Reserved MB | Shared Calls/Frame | Head Items/Update | Week2 Shared OK | Week2 Head OK | 25 FPS |"
    )
    lines.append(
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"
    )
    ...
    lines.extend(["", "## V7 FPS Profile", ""])
    lines.append(
        "| Sequence | Config | N Target | Transfer ms/update | Candidate ms/update | ReID crop ms/update | Identity ms/update | Accept ms/update | Hold ms/update | Appearance refresh ms/update | Debug/proof ms/update | Unbucketed ms/update |"
    )
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for row in timing_rows:
        identity_ms = None
    ...
    if identity_summary:
        ...
        lines.append("| Sequence | Config | N Target | Samples | Correct Rate | Jump Rate | Occluded Rate | Mean IoU |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
        for row in identity_summary:
            lines.append(
                f"| {row['sequence']} | {row['lorat_config']} | {row['target_tracks']} | {row['samples']} | "
                f"{bench.optional_float(row['correct_rate'], 3)} | {bench.optional_float(row['jump_rate'], 3)} | "
                f"{bench.optional_float(row['occluded_rate'], 3)} | {bench.optional_float(row['mean_iou'], 3)} |"
            )
    lines.extend(["", "## 25 FPS Capacity", ""])
    lines.append("| Sequence | Config | GPU Profile | Max N Sustaining 25 FPS |")
    lines.append("|---|---:|---:|---:|")
    for row in capacity_rows(timing_rows):
        lines.append(
            f"| {row['sequence']} | {row['lorat_config']} | {row['gpu_profile']} | "
...
def flush_outputs(
    ...
    write_summary_md(paths.summary_md, args, label, paths, timing_rows, area_rows, identity_rows)
    return area_rows
...
def main() -> int:
    ...
    timing_rows: List[V7TimingResult] = []
    ...
    print(f"LoRAT V7 benchmark run label: {label}", flush=True)
        ...
        for sequence_path in sequences:
            for target_tracks in track_counts:
                if STOP_REQUESTED:
                    ...
                    return 0
                preview_path = (
                    preview_video_path(paths, sequence_path.name, lorat_config, target_tracks, args.max_frames)
                    if args.save_video
                    else None
                )
                try:
                ...
                if STOP_REQUESTED:
                    print("Stop requested; flushed partial V7 benchmark outputs and exiting cleanly.", flush=True)
                    return 0
    ...
    print("Wrote LoRAT V7 benchmark files:", flush=True)
