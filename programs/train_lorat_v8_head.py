...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Validate on the first N training samples each epoch to prove the head can overfit/follow known objects. 0 disables.",
    )
...
def build_selected_banks(
    ...
    for item in objects:
        ...
        template_bbox, template_context_bbox = template_bboxes_for_augmentation(item, int(template_shape[1]), augmentation_spec)
        slot = trainer._template_slot_for_bbox(
            template_features,
            template_bbox,
            template_shape,
            item.target_kind,
...
def main() -> int:
    ...
    train_dataset = MOTFrameHeadDataset(
        args.dataset_root,
        args.split,
    ...
    val_dataset = MOTFrameHeadDataset(
        args.dataset_root,
        args.val_split,
        parse_class_ids(args.class_id),
        args.min_visibility,
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for sample_index in order:
            ...
            if args.hard_negative_loss_weight != 1.0 and hard_negative_mask.any():
                hard_negative_weights = torch.ones_like(objectness_map)
                hard_negative_weights[hard_negative_mask] = max(0.0, float(args.hard_negative_loss_weight))
