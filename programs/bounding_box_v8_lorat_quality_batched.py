...
WEEK2_PROOF_LOG_HEADER = (
    ...
    "profile_identity_resolve_ms,profile_identity_score_ms,profile_debug_output_ms,profile_accept_ms,"
...
class V8FeatureIdentityArbitrator(mot.LightweightIdentityArbitrator):
    ...
    def _output_feature_stack(self, outputs: Sequence[mot.LoRATSlotOutput]):
        ...
        for output in outputs:
            feature = getattr(output, "v8_feature", None)
            vector = self.normalize_feature(feature) if feature is not None else None
        ...
        if feature_dim is None:
            return None, np.zeros(len(outputs), dtype=bool)
    ...
    def _identity_score_matrices(
        ...
        candidate_features, candidate_valid = self._output_feature_stack(outputs)
    ...
    def track_feature_similarity(self, track: mot.TrackState, candidate) -> float:
        ...
        initial = getattr(track, "v8_initial_feature", None)
        if initial is not None:
    ...
    def score(
        ...
        if candidate is not None:
            appearance = self.track_feature_similarity(track, candidate)
            initial = getattr(track, "v8_initial_feature", None)
            if initial is not None:
            ...
            for other in all_tracks:
                if other.track_id == track.track_id or getattr(other, "v8_initial_feature", None) is None:
                    continue
                other_score = self.feature_similarity(getattr(other, "v8_initial_feature", None), candidate)
...
class V8QualityBatchedLoRATTracker:
    ...
    def _create_track(
        ...
        setattr(track, "v8_initial_feature", initial_feature)
    ...
    def _with_feature_appearance(
        ...
        setattr(enriched, "v8_feature", self._feature_mean_for_bbox(feature_map, output.bbox, frame_shape))
        return enriched
