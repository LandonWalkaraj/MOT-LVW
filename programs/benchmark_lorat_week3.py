...
def build_week3_args(user_args: Sequence[str]) -> List[str]:
    ...
    - sample identity every frame so occlusion survival is reported in frames
...
def main() -> int:
    ...
    print("Defaults: ReID ablation on, identity sampled every frame, full area observations on.", flush=True)
