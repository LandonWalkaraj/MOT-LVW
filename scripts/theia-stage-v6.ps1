...
param(
    ...
    [int]$Port = 222,
    [string]$BundleName = "theia_lorat_v6_bundle.tar.gz",
...
$StageRoot = Join-Path $ProjectRoot "outputs\theia-stage"
$PayloadRoot = Join-Path $StageRoot "mot-v6"
$BundlePath = Join-Path $StageRoot $BundleName
$SbatchPath = Join-Path $ProjectRoot "scripts\theia_v6_benchmark.sbatch"
...
Copy-RequiredPath "scripts\theia_v6_benchmark.sbatch"
...
& tar -czf $BundlePath -C $StageRoot "mot-v6"
...
Write-Host "  sbatch $RemoteWorkRoot/theia_v6_benchmark.sbatch"
