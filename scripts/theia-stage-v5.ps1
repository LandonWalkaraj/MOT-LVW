...
param(
    ...
    [int]$Port = 222,
    [string]$BundleName = "theia_lorat_v5_bundle.tar.gz",
...
$PayloadRoot = Join-Path $StageRoot "mot-v5"
$BundlePath = Join-Path $StageRoot $BundleName
$SbatchPath = Join-Path $ProjectRoot "scripts\theia_v5_benchmark.sbatch"
...
Copy-RequiredPath "scripts\theia_v5_benchmark.sbatch"
...
& tar -czf $BundlePath -C $StageRoot "mot-v5"
...
Write-Host "  sbatch $RemoteWorkRoot/theia_v5_benchmark.sbatch"
